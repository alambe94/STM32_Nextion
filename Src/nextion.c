		...
		while (Ring_Buffer_Get_Count(&UART_Ring_Buffer_Handle))
		{
			...
			if (data_received)
			{
				...
				case NEX_RET_NUMBER_HEAD:
					rx_number = (Nextion_In_Buffer[4] << 24) | (Nextion_In_Buffer[3] << 16) | (Nextion_In_Buffer[2] << 8) | (Nextion_In_Buffer[1] << 0);
...
__weak void Nextion_RX_Page_ID_Callback(uint8_t Page_ID)
{
}

/*********** called when Nextion send string ***********/
__weak void Nextion_RX_String_Callback(const char *str)
{
}

/*********** called when Nextion send number ***********/
__weak void Nextion_RX_Number_Callback(uint32_t Number)
{
}

/*********** called when Command Exe is finished ***********/
__weak void Nextion_CMD_Finished_Callback()
{
}
